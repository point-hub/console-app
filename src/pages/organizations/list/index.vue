...
const getOrganizations = async () => {
  ...
  const response = await axios.get('/v1/organizations', {
    params: {
      filter: {
        search: searchAll.value,
...
const openMenu = (organization: IOrganization, index: number) => {
          ...
          <tbody>
            ...
            <template v-if="!isLoading">
              <tr v-for="(organization, index) in organizations" :key="index">
                    ...
                    <template #content>
                      <base-card class="py-1! px-2! text-sm">
                        <div class="flex flex-col">
                          <router-link :to="`/organizations/${organization._id}`">
                          ...
                          <base-button
                            variant="text"
                            color="danger"
                            @click="openMenu(organization, index)"
                          >
                            <div class="flex gap-2 w-full">
                ...
                <td>
                  <router-link :to="`/organizations/${organization._id}`" class="text-blue">
                    {{ organization.name }}
                  </router-link>
                </td>
                <td>{{ formatDate(new Date(organization.created_date), 'dd-MM-yyyy') }}</td>
              </tr>
            </template>
          </tbody>
        </base-table>
